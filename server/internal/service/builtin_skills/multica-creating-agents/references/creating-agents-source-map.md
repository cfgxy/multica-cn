...
| DeerFlow ignores model / thinking_level / mcp_config | `pkg/agent/deerflow.go` `deerflowBackend.Execute` | The bridge pins its model from `DEERFLOW_ACP_MODEL` at process start and answers `session/set_model` and `session/set_config_option` with -32601, so both fields are logged as ignored instead of failing the task. A non-empty `mcp_config` is warned about and dropped because the bridge answers a non-empty `mcpServers` with -32602 rather than ignoring it. `ModelSelectionSupported("deerflow")` is false and `providerSupportsMcpConfig("deerflow")` is false, so neither knob is offered in the UI. |
| DeerFlow deployment root | `pkg/agent/deerflow.go` `deerflowHomeEnv` / `resolveDeerflowProcessDir` | `custom_env.MULTICA_DEERFLOW_HOME` selects the bridge process's working directory, because DeerFlow resolves its own `config.yaml` relative to cwd. The session's `cwd` param stays the task working directory — the two are separate concerns. Unset or non-directory values warn and fall back to the task workdir, where turns fail with backend-unavailable. |
| ZCode model and reasoning effort | `pkg/agent/zcode.go` `zcodeBackend.Execute` | `model` goes through the bridge's snake_case `session/set_model`, registered for this client; a failure fails the task rather than silently running the default model. `thinking_level` goes through the shared `applyACPEffortOption`, which reads the advertised config option (id `thought`, category `thought_level`) and its per-model vocabulary off the session instead of hard-coding kimi's `thinking`. |
