...
このリポジトリでは、メイン checkout と複数の worktree を同時に実行できます。PostgreSQL コンテナとメインデータベース `multica` を共有し、ポートとプロセス設定を分離します。
...
`make setup-worktree` は `.env.worktree` を生成します。ポートとプロセス識別子はパスから決まり、`POSTGRES_DB` は `multica` のままです。再び起動するには次を実行します。
...
<Callout type="info">
worktree 間では PostgreSQL コンテナと `multica` を共有します。worktree ごとに新しい Compose project を起動しないでください。まず `.env.worktree` の `PORT` と `FRONTEND_PORT` を確認し、破壊的なテストには使い捨てのデータベースを使用してください。
</Callout>
