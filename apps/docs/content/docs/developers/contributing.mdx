...
The repository supports running the primary checkout and multiple worktrees at the same time. They share the PostgreSQL container and the main database `multica`, while using separate ports and process profiles.
...
`make setup-worktree` generates `.env.worktree`; ports and process identity are derived from the path, while `POSTGRES_DB` remains `multica`. To start it again:
...
<Callout type="info">
Different worktrees intentionally share the PostgreSQL container and `multica`. Do not start a new Compose project for every worktree. Check `PORT` and `FRONTEND_PORT` in `.env.worktree`, and use a disposable database for destructive tests.
</Callout>
