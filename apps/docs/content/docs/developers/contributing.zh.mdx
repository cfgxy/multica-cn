...
仓库支持主 checkout 和多个 worktree 同时运行。它们共用 PostgreSQL 容器和主库 `multica`，但使用不同端口和进程配置。
...
`make setup-worktree` 会生成 `.env.worktree`；端口和进程标识根据路径确定，`POSTGRES_DB` 保持为 `multica`。再次启动：
...
<Callout type="info">
不同 worktree 会共用 PostgreSQL 容器和 `multica`。不要为每个 worktree 启动新的 Compose project；先检查 `.env.worktree` 中的 `PORT` 和 `FRONTEND_PORT`，破坏性测试必须使用一次性数据库。
</Callout>
