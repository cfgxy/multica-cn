...
| `ATTACHMENT_DOWNLOAD_URL_TTL` | `24h` | 署名付きダウンロード URL の有効期間 |
...
設定後、このレイヤーの利用者はちょうど 3 つです。会話の自動タイトル生成（新しいチャットの最初のユーザーメッセージ）、会話のフォローアップ質問（会話末尾の数メッセージ）、そして Prompt 品質ダッシュボードの D3 次元（ワークスペース自身が組み立てたシステムプロンプト。メールアドレスは事前にマスクされ、資格情報スキャンが 1 件でも検出すれば呼び出し全体を中止します）。API キーと base URL の両方を空にしておく構成も完全にサポートされています。その場合このレイヤーは無効となり、上流へのリクエストは一切発生せず、チャットはクライアント側で生成したタイトルのまま、フォローアップ質問のボタンは表示されず、D3 は「未評価」と表示され、残る 6 次元はこれまでどおりプラットフォームのデータから算出されます。
...
各 AI コーディングツールは `MULTICA_<PROVIDER>_PATH` でコマンドパスを上書きでき、モデル上書き対応ツールでは `MULTICA_<PROVIDER>_MODEL` も利用できます。QwenPaw と MiniMax Code にはモデル変数がなく、MiniMax Code のパス変数は `MULTICA_MCODE_PATH` です。詳しくは [AI コーディングツール比較](/providers#モデルの取得元) を参照してください。DeepSeek Harness は `MULTICA_DSH_PATH` と `MULTICA_DSH_MODEL` に対応しています（値は dsh のモデルカタログにあるモデル ID、例: `deepseek-official/deepseek-chat`）。ZeroClaw は `MULTICA_ZEROCLAW_PATH` に対応しますが、モデル変数はありません。モデルは ZeroClaw のエージェント設定で管理します。DeerFlow は `MULTICA_DEERFLOW_PATH` に対応しますが、モデル変数はありません(ブリッジが起動時に自身の `DEERFLOW_ACP_MODEL` からモデルを固定します)。さらに `MULTICA_DEERFLOW_HOME` を読み取り、この値は DeerFlow のデプロイ ルートを指す必要があります。DeerFlow は作業ディレクトリを基準に自身の `config.yaml` を解決するため、ブリッジ プロセスはそのディレクトリで起動されます(セッションを開く対象はタスクの作業ディレクトリのままです)。ZCode は `MULTICA_ZCODE_PATH` と `MULTICA_ZCODE_MODEL`(zcode カタログのモデル ID)に対応します。マシン全体のデフォルト引数 `MULTICA_<PROVIDER>_ARGS` は、現在 Claude Code、Codex、CodeBuddy、Qwen Code、QwenPaw の 5 ツールに対応しています。対応する変数は `MULTICA_CLAUDE_ARGS`、`MULTICA_CODEX_ARGS`、`MULTICA_CODEBUDDY_ARGS`、`MULTICA_QWEN_ARGS`、`MULTICA_QWENPAW_ARGS` です。例:
