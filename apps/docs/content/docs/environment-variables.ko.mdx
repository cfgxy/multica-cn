...
| `ATTACHMENT_DOWNLOAD_URL_TTL` | `24h` | 서명된 다운로드 주소의 유효 기간 |
...
설정한 뒤 이 계층의 소비자는 정확히 셋입니다. 대화 자동 제목(새 대화의 첫 사용자 메시지), 대화 후속 질문(대화 끝부분의 몇 개 메시지), 그리고 Prompt 품질 대시보드의 D3 차원(워크스페이스가 직접 조립한 시스템 프롬프트로, 이메일 주소를 먼저 마스킹하고 자격 증명 스캔에서 하나라도 발견되면 호출 전체를 포기합니다). API 키와 base URL을 모두 비워 두는 것도 완전히 지원되는 구성입니다. 이 경우 계층이 비활성화되어 업스트림 요청을 전혀 보내지 않고, 대화는 클라이언트가 만든 제목을 유지하며, 후속 질문 버튼은 나타나지 않고, D3는 "평가 안 됨"으로 표시되며 나머지 여섯 차원은 평소대로 플랫폼 데이터로 계산됩니다.
...
각 AI 코딩 도구는 `MULTICA_<PROVIDER>_PATH`로 명령 경로를 덮어쓸 수 있으며 모델 재정의를 지원하는 도구는 `MULTICA_<PROVIDER>_MODEL`도 사용할 수 있습니다. QwenPaw와 MiniMax Code에는 모델 변수가 없고 MiniMax Code의 경로 변수는 `MULTICA_MCODE_PATH`입니다. 자세한 내용은 [AI 코딩 도구 비교](/providers#모델-출처)를 참고하세요. DeepSeek Harness는 `MULTICA_DSH_PATH`와 `MULTICA_DSH_MODEL`을 지원합니다(값은 dsh 모델 카탈로그의 모델 ID, 예: `deepseek-official/deepseek-chat`). ZeroClaw는 `MULTICA_ZEROCLAW_PATH`를 지원하지만 모델 변수는 없습니다. 모델은 ZeroClaw의 에이전트 설정에서 관리합니다. DeerFlow는 `MULTICA_DEERFLOW_PATH`를 지원하지만 모델 변수는 없습니다. 브리지가 시작할 때 자체 `DEERFLOW_ACP_MODEL`에서 모델을 고정합니다. 또한 `MULTICA_DEERFLOW_HOME`을 읽으며, 이 값은 DeerFlow 배포 루트를 가리켜야 합니다. DeerFlow가 작업 디렉터리를 기준으로 자체 `config.yaml`을 해석하기 때문에 브리지 프로세스는 그 디렉터리에서 시작됩니다(세션을 여는 대상은 여전히 작업 디렉터리입니다). ZCode는 `MULTICA_ZCODE_PATH`와 `MULTICA_ZCODE_MODEL`(zcode 카탈로그의 모델 ID)을 지원합니다. 컴퓨터 전체 기본 인수 `MULTICA_<PROVIDER>_ARGS`는 현재 Claude Code, Codex, CodeBuddy, Qwen Code, QwenPaw 다섯 도구를 지원합니다. 해당 변수는 `MULTICA_CLAUDE_ARGS`, `MULTICA_CODEX_ARGS`, `MULTICA_CODEBUDDY_ARGS`, `MULTICA_QWEN_ARGS`, `MULTICA_QWENPAW_ARGS`입니다. 예시:
