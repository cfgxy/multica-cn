...
| `ATTACHMENT_DOWNLOAD_URL_TTL` | `24h` | 签名下载地址的有效期 |
...
配置之后，这一层恰好有三个消费方：对话自动标题（新会话的第一条用户消息）、对话追问建议（对话末尾若干条消息），以及 Prompt 质量看板的 D3 维度（工作空间自身拼接出的系统提示词，先掩码邮箱地址，凭据扫描一旦命中就整次放弃调用）。API key 与 base URL 同时留空是完整支持的配置：该层被禁用，不发出任何上游请求，对话保留客户端生成的标题，追问按钮不出现，D3 显示为「未评分」，其余六个维度照常由平台数据计算。
...
各 AI 编程工具可以使用 `MULTICA_<PROVIDER>_PATH` 覆盖命令路径；支持模型覆盖的工具还可以使用 `MULTICA_<PROVIDER>_MODEL`。QwenPaw 和 MiniMax Code 没有模型变量，因为 Multica 不会向它们传模型；MiniMax Code 的路径变量是 `MULTICA_MCODE_PATH`。详见 [AI 编程工具对比](/providers#模型来源)。DeepSeek Harness 支持 `MULTICA_DSH_PATH` 和 `MULTICA_DSH_MODEL`（取值为 dsh 模型目录中的模型 id，例如 `deepseek-official/deepseek-chat`）。ZeroClaw 支持 `MULTICA_ZEROCLAW_PATH`，但没有模型变量；模型由 ZeroClaw 的智能体配置管理。DeerFlow 支持 `MULTICA_DEERFLOW_PATH`，但没有模型变量：桥接器在启动时从自身的 `DEERFLOW_ACP_MODEL` 固定模型。它还会读取 `MULTICA_DEERFLOW_HOME`，该变量必须指向 DeerFlow 部署根目录——桥接进程在该目录下启动，因为 DeerFlow 按工作目录解析自己的 `config.yaml`，而任务工作目录仍是会话打开时使用的目录。ZCode 支持 `MULTICA_ZCODE_PATH` 和 `MULTICA_ZCODE_MODEL`（取值为 zcode 模型目录中的模型 ID）。全机默认参数 `MULTICA_<PROVIDER>_ARGS` 目前支持 Claude Code、Codex、CodeBuddy、Qwen Code 和 QwenPaw 五款工具。对应变量是 `MULTICA_CLAUDE_ARGS`、`MULTICA_CODEX_ARGS`、`MULTICA_CODEBUDDY_ARGS`、`MULTICA_QWEN_ARGS` 和 `MULTICA_QWENPAW_ARGS`。例如：
